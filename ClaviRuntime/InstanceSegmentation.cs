...
namespace ClaviRuntime
{
    public class InstanceSegmentation
    {
        ...
        public void Process(string imagePath, float threshold = 0.7F, double opacity = 0.5)
        {
            resultsList = new List<InstanceSegmentationResults>();
            Bitmap hmResult;
            ...
            var input = new DenseTensor<float>(MatToList(imageFloat), new[] { 1, 3, dims[2], dims[3] });
            ...
            {
                var inferenceResult = results.First().AsDictionary<string, float>();
        ...
        public static Mat ConvertInstanceSegmentationResult(int id, float[] pred, int[] pred_dim, float threshold)
        {
            ...
            for (int batch = 0; batch < pred_dim[0]; batch++)
            {
                for (int h = 0; h < pred_dim[2]; h++)
                {
                    ...
                    {
                        int idx = (batch * pred_dim[0] * pred_dim[2] * pred_dim[3]) + (id * pred_dim[2] * pred_dim[3]) + (h * pred_dim[3]) + w;
